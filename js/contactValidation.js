...
$(document).ready(function(){
	$("#btn_submit").on('click',function(){
		...
		var name = $("#fsurname").val();
		if(name == ''){
		    status = false;
		   $("#error-fsurname").html('please enter surname');
		}else{
		    var regex = /^[a-zA-Z]+(([',. -][a-zA-Z ])?[a-zA-Z]*)*$/;
            if (regex.test(surname)==false) {
                status = false;
                $("#error-fsurname").html('please enter valid surname');
            }
            else {
            	 status = true;
		        $("#error-fsurname").html(' ');
		...
		if(status == true){
		var FormData = $("#cnt_frm").serialize();
		$("#email_status").html('');
		$("#email_not_send").html('');
		$("#btn_send").text("Sending Email...");
		$.ajax({
	            type: "POST",
	            data: FormData,
